...
import com.blazeloader.api.util.render.MCColor;
...
public class BLRenderBlocks {
    ...
    private boolean renderStandardBlockWithAmbientOcclusionPartial(Block block, int x, int y, int z, float multR, float multG, float multB) {
        ...
        int originalBrightness = block.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z),
                usedBrightness;
        ...
        if (block.shouldSideBeRendered(renderBlocks.blockAccess, x, y - 1, z, 0)) {
            ...
            if (renderBlocks.renderMinY <= 0.0D || !renderBlocks.blockAccess.getBlock(x, y - 1, z).isOpaqueCube()) {
                usedBrightness = block.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y - 1, z);
            } else {
        ...
        if (block.shouldSideBeRendered(renderBlocks.blockAccess, x, y, z - 1, 2)) {
            ...
            if (renderBlocks.renderMinZ <= 0.0D || !renderBlocks.blockAccess.getBlock(x, y, z - 1).isOpaqueCube()) {
                usedBrightness = block.getMixedBrightnessForBlock(renderBlocks.blockAccess, x, y, z - 1);
            } else {
                usedBrightness = originalBrightness;
            }
        ...
        if (block.shouldSideBeRendered(renderBlocks.blockAccess, x - 1, y, z, 4)) {
            ...
            if (renderBlocks.renderMinX <= 0.0D || !renderBlocks.blockAccess.getBlock(x - 1, y, z).isOpaqueCube()) {
                usedBrightness = block.getMixedBrightnessForBlock(renderBlocks.blockAccess, x - 1, y, z);
            } else {
                usedBrightness = originalBrightness;
            }
    ...
    private boolean renderStandardBlockWithColorMultiplier(Block block, int x, int y, int z, float multR, float multG, float multB) {
        ...
        boolean result = renderFaceOld(tessellator, block, renderGrass, mixedBrightness, x, 0, y, -1, z, 0, colors[0], mult, 0);
    ...
    private boolean renderFaceOld(Tessellator tessellator, Block par1Block, boolean renderGrass, int mixedBrightness, int x, int xOff, int y, int yOff, int z, int zOff, float[] rgb, float[] rgbMult, int side) {
        if (par1Block.shouldSideBeRendered(renderBlocks.blockAccess, x + xOff, y + yOff, z + zOff, side)) {
            tessellator.setBrightness(checkBoundsForSide(side) ? mixedBrightness : par1Block.getMixedBrightnessForBlock(renderBlocks.blockAccess, x + xOff, y + yOff, z + zOff));
            ...
            if (!renderBlocks.hasOverrideBlockTexture()) {
                if (RenderBlocks.fancyGrass && side > 1 && texture != null && texture.getIconName().equals("grass_side")) {
                    ...
                    RenderFaceforSide(par1Block, x, y, z, BlockGrass.getIconSideOverlay(), side);
                    return true;
                }
    ...
    private boolean checkBoundsForSide(int side) {
        switch (side) {
            case 0:
                return renderBlocks.renderMinY > 0.0D;
            case 1:
                return renderBlocks.renderMaxY < 1.0D;
            case 2:
                return renderBlocks.renderMinZ > 0.0D;
            case 3:
                return renderBlocks.renderMaxZ < 1.0D;
            case 4:
                return renderBlocks.renderMinX > 0.0D;
            case 5:
                return renderBlocks.renderMaxX < 1.0D;
        }
        return false;
    }
    ...
    public void RenderFaceforSide(Block block, double x, double y, double z, IIcon icon, int side) {
        switch (side) {
            case 0:
                renderBlocks.renderFaceYNeg(block, x, y, z, icon); //bottom
                return;
            case 1:
                renderBlocks.renderFaceYPos(block, x, y, z, icon); //top
                return;
            case 2:
                renderBlocks.renderFaceZNeg(block, x, y, z, icon); //east
                return;
            case 3:
                renderBlocks.renderFaceZPos(block, x, y, z, icon); //west
                return;
            case 4:
                renderBlocks.renderFaceXNeg(block, x, y, z, icon); //north
                return;
            case 5:
                renderBlocks.renderFaceXPos(block, x, y, z, icon); //south
        }
    }
        ...
        if (!renderBlocks.hasOverrideBlockTexture()) {
            if (side > 1 && RenderBlocks.fancyGrass) {
                ...
                if (var22.getIconName().equals("grass_side")) {
                    ...
                    RenderFaceforSide(block, (double) x, (double) y, (double) z, BlockGrass.getIconSideOverlay(), side);
                    return;
                }
            }
    ...
    public void renderWithThickness(Block block, float thickness, float x, float y, float z, int swap, int diagonal) {
        ...
        tess.setBrightness(block.getMixedBrightnessForBlock(renderBlocks.blockAccess, (int) x, (int) y, (int) z));
    ...
    private static void addVertexWithUVSwap(Tessellator tess, double xOff, double yOff, double zOff, double u, double v, int swap) {
        ...
        switch (swap) {
            case 1:
                tess.addVertexWithUV(xOff, zOff, yOff, u, v);
                return;
            case 2:
                tess.addVertexWithUV(yOff, xOff, zOff, u, v);
                return;
            case 3:
                tess.addVertexWithUV(yOff, zOff, xOff, u, v);
                return;
            case 4:
                tess.addVertexWithUV(zOff, xOff, yOff, u, v);
                return;
            case 5:
                tess.addVertexWithUV(zOff, yOff, xOff, u, v);
                return;
        }
